...
describe "lists donations", :type => :feature do
  before :each do
    Donor.create(:email => 'testuser@berthartmann.com', :password => 'password')
    visit '/donors/sign_in'
    within("#new_donor") do
  ...
  it "should list donations" do
    visit "/"
    expect(page.find(:xpath, '//div[@id = "donations"]')).to have_content "Physical donation test"
    expect(page.find(:xpath, '//div[@id = "donations"]')).to have_content "Voucher donation test"
    expect(page.find(:xpath, '//div[@id = "donations"]')).to have_content "Experience donation test"
  end
end
